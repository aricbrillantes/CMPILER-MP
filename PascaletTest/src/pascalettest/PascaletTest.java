...
public class PascaletTest 
{
    ...
    public static void main(String[] args) 
    {
    	...
    	CharStream in = CharStreams.fromFileName("X22_BroDudeTsong.pas");
        PASCALetLexer lexer = new PASCALetLexer(in);
	            ...
	            {	
	            	...
	            	if(line.contains("readln"))
	            	{
	            	    ...
	            	    for(int i=0;i<splitVarsArr.length;i++)
	            		{
	            			if(scopeflag==1)
	            			{
	            				if(funcproc.containsKey(splitVarsArr[i]) && scan.hasNextLine())
	            				{
	            					tempscan = scan.nextLine();
	            					funcproc.get(splitVarsArr[i])[0] = tempscan;
	            				}
	            				
	            				else if(global.containsKey(splitVarsArr[i])&& scan.hasNextLine())
	            				{
	            					tempscan = scan.nextLine();
	            			...
	            			{
	            				if(main.containsKey(splitVarsArr[i])&& scan.hasNextLine())
	            				{
	            					tempscan = scan.nextLine();
	...
	//            	            			System.out.println(toEval);
            ...
            System.out.println("\n" + parser.getNumberOfSyntaxErrors() + " error/s found pls fix :(");
            hasSyntaxError = true;
         }
